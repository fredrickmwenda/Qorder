 ...
 // const navigate = useNavigate();
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      // If the user is successfully authenticated, we set a loggedIn flag in local storage or session storage to keep track of the user's login status.
      // await auth.signInWithEmailAndPassword(email, password);
      const userCredential = await signInUser(email, password);
      const user = userCredential.user;
      localStorage.setItem('loggedIn', 'true');
      setSuccessMsg('You have successfully logged in!');
      navigate('/cart'); // assuming you have access to history object
    } catch (error) {
      console.log(error);
          ...
          <div class="w-full px-4">
            ...
            >
              ...
              <form onSubmit={handleSubmit}>
                ...
                <div class="mb-10">
                  <button type="submit" className='bg-transparent hover:bg-blue-500 text-blue-700 w-full cursor-pointer rounded-md border  py-3 px-5 text-base  font-semibold hover:text-white border-blue-500'>Sign In</button>
                </div>
              </form>
              {errorMsg&&<>
                <br></br>
